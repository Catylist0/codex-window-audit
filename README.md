...
The build uses local `Europe/London` dates, merges reset timestamps within 120 seconds, and includes five-hour windows peaking at 80% or above. Prices are standard short-context API equivalents, not charges. Before the local price ledger begins, GPT-5.5 and GPT-5.6 Sol rates are reconstructed from official launch pricing; GPT-5.5 cached-input continuity is an explicit assumption. Unpriced `codex-auto-review` usage makes four July dollar totals lower bounds.
