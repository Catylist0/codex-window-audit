(() => {
  ...
  function renderTable(rows) {
    ...
    rows.forEach(row=>{
      ...
      tr.append(makeCell(`${row.date} · ${row.time}`),makeCell(`${row.maxPercent.toFixed(0)}%`),makeCell((row.priceComplete ? "" : "≥")+money(row.recordedUsd)),makeCell((row.priceComplete ? "" : "≥")+money(row.fullWindowUsd)));
      ...
      const mixCell=document.createElement("td");mixCell.className="mix"; const mixText=document.createElement("span");mixText.className="mix-text";mixText.textContent=mix(row).filter(x=>x.percent>=0.5).map(x=>`${modelNames[x.name]||x.name} ${Math.round(x.percent)}%`).join(" · ");
  ...
  function drawChart(canvas, rows, type) {
    ...
    const minX=type==="timeline" ? Math.min(...records.map(r=>r.timestamp)) : 75;
    const maxX=type==="timeline" ? Math.max(...records.map(r=>r.timestamp)) : 101;
    ...
    if(type==="timeline"){
      ...
      [80,85,90,95,100].forEach(value=>{const xx=X(value);ctx.fillStyle="#a6afba";ctx.fillText(value+"%",xx,box.bottom+11);});
    }
    const points=[];
    rows.forEach(row=>{
      const xx=X(type==="timeline"?row.timestamp:row.maxPercent), yy=Y(type==="timeline"?row.fullWindowUsd:row.recordedUsd);
  ...
  function attachTooltip(canvas,tooltip) {
    canvas.addEventListener("mousemove",event=>{
      ...
      const r=nearest.row;tooltip.textContent=`${r.date} ${r.time} · ${r.maxPercent.toFixed(0)}% used · ${money(r.recordedUsd)} recorded · ${money(r.fullWindowUsd)} per 100% · ${modelNames[r.dominant]||r.dominant}${r.observedFull?" · observed":" · extrapolated"}${r.priceComplete?"":" · partial price"}`;
  ...
  function render(){const rows=selected();document.getElementById("filter-count").textContent=`Showing ${rows.length} of ${records.length} windows`;renderTable(rows);drawChart(document.getElementById("timeline-chart"),rows,"timeline");drawChart(document.getElementById("scatter-chart"),rows,"scatter");}
  ...
  document.getElementById("download-csv").addEventListener("click",()=>{
    const header=["date","time_uk","minimum_usage_percent","max_usage_percent","snapshot_count","recorded_usd","usd_per_100_percent","observed_full","extrapolated","price_quality","total_tokens","gpt_5_5_token_share","gpt_5_6_sol_token_share","gpt_5_6_luna_token_share","gpt_6_astra_token_share","auto_review_token_share"];
    const csv=[header.join(",")];for(const r of selected()){const share=name=>r.totalTokens?(r.modelTokens[name]||0)/r.totalTokens:"";csv.push([r.date,r.time,r.minPercent,r.maxPercent,r.snapshotCount,r.recordedUsd,r.fullWindowUsd,r.observedFull,!r.observedFull,r.priceQuality,r.totalTokens,share("gpt-5.5"),share("gpt-5.6-sol"),share("gpt-5.6-luna"),share("gpt-6-astra"),share("codex-auto-review")].join(","));}
